...
```sh
bash -c "$(curl -fsSL https://raw.githubusercontent.com/aa123330/QL-/main/lang1.sh)"
```
- 国内鸡地址，执行下面一键命令安装青龙+依赖+任务+NVJDC自由选择（安装完毕后再次使用命令可以对应用进行升级）
```sh
bash -c "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/lang1.sh)"
```
#
- 单独安装rabbit一键脚本
```sh
bash -c "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/rabbit.sh)"
```
- 单独安装JDX
```sh
...
```sh
bash -c "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/nvjdc.sh)"
```
- 单独安装阿东一键脚本（免费有限制，一天扫码2次，低调使用，如有批量需求，请捐赠原作者）
```sh
bash -c "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/adong/adong.sh)"
```
...
- 一键单独安装docker
```sh
bash -c "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/docker.sh)"
```

- 一键安装单独青龙的依赖
```sh
docker exec -it qinglong bash -c  "$(curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/aa123330/QL-/main/npm.sh)"
```
#### 🚩 单独F2拉库
F2库
```sh
...
大灰狼备份库
```sh
ql repo https://github.com/aa123330/f2.git "jd_|jx_|gua_|jddj_|jdCookie" "activity|backUp" "^jd[^_]|USER|function|utils|sendNotify|ZooFaker_Necklace.js|JDJRValidator_|sign_graphics_validate|ql|JDSignValidator"
```
