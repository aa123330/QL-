...
function qinglong_port() {
  ...
  *)
  ...
  if [[ "${Api_Client}" == "true" ]]; then
     read -p " rabbit面板名称，可中文可英文(直接回车默认：rabbit): " RABBITNAME && printf "\n"
     export RABBITNAME=${RABBITNAME:-"rabbit"}
     read -p " 请输入您想设置的rabbit面板端口(直接回车默认：5701): " JDC_PORT && printf "\n"
...
function linux_rabbit() {
 ...
 # else
  ...
  timeout 9 docker logs -f rabbit |tee ${Home}/build.log
  if [[ `grep -c "启动成功" ${Home}/build.log` -ge '1' ]] || [[ `grep -c "NETJDC started" ${Home}/build.log` -ge '1' ]]; then
    print_ok "rabbit安装 完成"
    ECHOYY "rabbit配置已自动配置完成，如您需修改可至 ${Config}/Config.json 修改!"
  else
    print_error "rabbit安装失败"
    exit 1
  fi
  rm -rf ${Home}/build.log
...
rm -rf /root/Rabbit > /dev/null
  ...
  if [[ `docker images | grep -c "rabbit"` == '0' ]]; then
    print_ok "rabbit镜像卸载完成"
  else
    print_error "rabbit镜像卸载失败，再次尝试删除"
  fi
  cd ${Current}
  ECHOG "更新镜像，请耐心等候..."
